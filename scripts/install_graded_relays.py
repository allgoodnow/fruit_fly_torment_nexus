"""Install exact annotation-bound L1/L2 cohorts without changing synaptic weights."""
import hashlib
import json
...
def main():
    ...
    registry=attach_graded_relays(graph.circuits,pd.read_feather(raw),graph.ids)
    ...
    report={'format':'nexus-graded-relay-evidence-1','annotations':record,'cohort':registry['graded_relays'],
            'manifest_sha256':digest(pack/'manifest.json'),'weights_unchanged':True}
    (ROOT/'experiments/graded-relay-evidence-v1.json').write_text(json.dumps(report,indent=2)+'\n')
    (ROOT/'data/manifests/male-cns-runtime-v9.json').write_text(json.dumps(manifest,indent=2)+'\n')
