...
class InterventionPanel(QWidget):
    def __init__(self, brain_panel, *, coupled=True):
        ...
        self.pause.clicked.connect(lambda: brain_panel.send('running', not brain_panel.telemetry.get('running', False)))
        ...
        self.reposition = QPushButton('Reposition body only')
        self.reposition.setToolTip('Restore upright posture and pause. Neural state and simulation time are preserved.')
        self.reposition.clicked.connect(lambda: brain_panel.send('reposition_body'))
        self.reposition.setVisible(False)
        controls.addWidget(self.reposition)
    ...
    def accept_snapshot(self, packet):
        ...
        self.pause.setText('Pause' if packet['running'] else 'Resume')
        recovery = packet.get('recovery', {})
        latest = recovery.get('episodes', [])[-1:]
        if latest and latest[0].get('release_ms') is not None and recovery.get('phase') == 'observing':
            self.status.setText(self.status.text()+' · observing after release')
        self.reposition.setVisible(self.coupled and packet.get('body_upright') is not None and packet['body_upright'] < .2)
