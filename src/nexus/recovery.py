...
class RecoveryMonitor:
    RULES = {'neural_hz_per_neuron_max': .1, 'escape_max': .05,
             'disruption_max': .05, 'steering_hz_max': 10.,
    ...
    def observe(self, tick, spikes, neurons, *, escape, disruption, steering_hz, upright):
        ...
        if not all(math.isfinite(v) for v in (escape, disruption, steering_hz)):
